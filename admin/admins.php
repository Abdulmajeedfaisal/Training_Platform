...
if (isset($_POST['delete'])) {
   ...
   $verify_delete = $conn->prepare("SELECT * FROM `admins` WHERE id = :id");
   $verify_delete->bindParam(':id', $delete_id);
   $verify_delete->execute();

   if ($verify_delete->rowCount() > 0) {
      $delete_admin = $conn->prepare("DELETE FROM `admins` WHERE id = :id");
      $delete_admin->bindParam(':id', $delete_id);
...
<body>
    ...
    <section class="grid" style="direction: rtl;">
        ...
        <form action="" method="POST" class="search-form">
            <input type="text" name="search_box" placeholder="بحث عن مشرف..." maxlength="100" required>
        ...
        <div class="box-container">
         ...
         ?>
            ...
            <div class="box" style="order: -1;">
                <p>اسم المشرف : <span><?= $fetch_admins['name']; ?></p>
                <a href="update.php" class="option-btn">تحديث الحساب</a>
                <a href="register.php" class="btn">اضافة مشرف جديد</a>
            </div>
            <?php } else { ?>
            <div class="box">
                <p>اسم المشرف : <span><?= $fetch_admins['name']; ?></p>
