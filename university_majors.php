    ...
    <section class="listings" style="direction: rtl;">
        ...
        <div class="box-container">
            ...
            if (isset($_POST['search_box']) or isset($_POST['search_btn'])) {
                ...
                $select_properties = $conn->prepare("SELECT * FROM `property` WHERE  (property_name LIKE '%{$search_box}%' OR address LIKE '%{$search_box}%')  AND type = :property_type ORDER BY date DESC");
                $select_properties->bindParam(':property_type', $get_type);
                $select_properties->execute();
            } else {
                $select_properties = $conn->prepare("SELECT * FROM `property` WHERE type = :property_type  ORDER BY date DESC");
                $select_properties->bindParam(':property_type', $get_type);
                $select_properties->execute();;
            }
            $total_images = 0;
